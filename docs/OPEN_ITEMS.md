...
day; the workflow audit refilled it within the hour with #32, which was then
fixed too. An empty table is a claim about the moment, not a promise.
...
| 14 | `PWD` is defined inside `.env`, and the bind mounts interpolate it — **FIXED 2026-08-30** | The premise this was parked on for five days was backwards. `.env` does not feed `${PWD}`: compose gives the **shell environment precedence over the `.env` file**, and a shell always exports `PWD`, so the `PWD=~` sitting in `.env` could never take effect and every mount followed the caller's working directory instead. Measured 2026-08-30 — `docker compose -f <repo>/docker-compose.yml config` run from `/tmp` resolved the bastion's read-only `/etc/passwd`, `/etc/shadow`, `/etc/group`, `/etc/ssh` and `/home` mounts, and the gateway's `~/.ssh`, to `/tmp/data/...` and `/tmp/ssh/`, and validated without a murmur. All fifteen mounts across both compose files are now relative (`./ssh`), which compose resolves against the project directory; re-running the same command from `/tmp` after the change resolved them to the project. The `${PWD}` in the README's compose examples went with them; the `$PWD` in its **shell** commands (`chmod`, `docker run -v`) is correct and stays. `tests/unit/compose.bats` fails on `PWD` appearing in either compose file. Nothing needs removing from `.env-dist` — it never declared the key, which is part of why this hid. |
